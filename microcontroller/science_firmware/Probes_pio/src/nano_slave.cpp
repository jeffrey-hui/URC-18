...
void loop(void) {
    ...
    if (millis(pHArrayIndex) - samplingTime > samplingInterval) {
        pHArray[] = analogRead(pHPin);
        if (pHArray == ArrayLength)pHArray = 0;
    ...
    if (serialDataAvailable() > 0) {
        ...
        ecCalibration(
                modeIndex);                                                           // If the correct calibration command is received, the calibration function should be called.
    }

    static unsigned long analogSampleTimepoint = millis();
    if (millis() - analogSampleTimepoint >
        30U) {                                           //every 30ms,read the analog value from the ADC
        analogSampleTimepoint = millis();
        analogBuffer[analogBufferIndex] = analogRead(
                ecSensorPin);                          //read the analog value and store into the buffer,every 40ms
        analogBufferIndex++;
    ...
    if (millis() - tempSampleTimepoint >
        850U) {                                            // every 1.7s, read the temperature from DS18B20
    ...
    if (millis() - printTimepoint > 1000U) {
        ...
        float TempCoefficient = 1.0 + 0.0185 * (ec_tempC -
                                                25.0);                           //temperature compensation formula: fFinalResult(25^C) = fFinalResult(current)/(1.0+0.0185*(fTP-25.0));
        float CoefficientVolatge = (float) averageVoltage / TempCoefficient;
        if (CoefficientVolatge < 150)
            Serial.println(
                    F("EC: No solution!"));                                          //25^C 1413us/cm<-->about 216mv  if the voltage(compensate)<150,that is <1ms/cm,out of the range
